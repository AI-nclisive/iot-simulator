 ...
 * stream. The Modbus server runtime publishes connect/disconnect events here;
 * the gRPC service registers one observer per supervisor subscription.
 * Mirrors {@code worker-opcua}'s {@code ClientEventHub}.
 *
 * <p>Events are point-in-time and not buffered — one published while no stream is
 * open is dropped. See openspec/specs/worker-contract/spec.md.
 */
final class ClientEventHub {
    ...
    /**
     * Publishes an event to every open stream. Each subscriber's {@code onNext}
     * is serialized on a lock the hub owns; a cancelled or failing stream is
     * dropped.
     */
