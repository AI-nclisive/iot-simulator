...
- [x] IS-136 [BE] ✅ Recording value filter API — extend `GET .../values` with query params: `search` (ILIKE on path/nodeId), `quality` (comma-separated GOOD/UNCERTAIN/BAD), `from`/`to` (ISO-8601 instants); total reflects filtered count; keyset cursor pagination preserved. FE half: UI-122.
- [x] IS-137 [BE] ✅ Recording schema API — `GET /api/v1/projects/{pid}/recordings/{id}/schema` returns all schema_nodes for the recording's linked schema (via data_source_id + schema_version); response: `{ nodes: [{nodeId, parentId, path, kind, dataType, valueRank, access, unit, description}] }`, ordered by path. FE half: UI-123.
