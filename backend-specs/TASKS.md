...
- [x] IS-188 [BE] ✅ [source] Create-from-scan fails on real devices exposing Variable-of-Variable nodes (e.g. AnalogItemType `EURange`/`EngineeringUnits`/`Quality` Properties, or a structured value's `HasComponent` sub-Variables) — `OpcUaDiscovery.browse()` emitted every discovered node with its literal address-space parent as `parentId`, but a Variable is never a valid schema parent (`SchemaNodeValidator` only allows `FOLDER`/`OBJECT`), so any such node made the whole `createFromScan`/`applyRescan` call throw `parent must be a FOLDER or OBJECT: <nodeId>`. Rather than dropping this data (which would make a scanned schema not faithfully reproduce the real device), such nodes are now re-homed onto the nearest `FOLDER`/`OBJECT` ancestor — covering both `HasProperty` and `HasComponent` children of a Variable, at any depth. `VarDef`/`SchemaNamespace` gained a test-only `referenceType` seam so `OpcUaDiscoveryIT` can build real `HasProperty`/`HasComponent` fixtures. Reported against a real device: `parent must be a FOLDER or OBJECT: ns=6;s=MyLevel.Alarm/0:Quality`.
- [ ] IS-189 [BE] ⬜ [schema] Materialize a Variable's Property/Component children with their real reference type, for exact device reproduction — IS-188 fixed the crash by re-homing a Variable's `HasProperty`/`HasComponent` children onto the nearest FOLDER/OBJECT ancestor, which preserves the *data* but not the *topology*: a real device's "Alarm --HasProperty--> Quality" reproduces in the simulated server as "Plant --Organizes--> Quality" (a sibling of Alarm, not its Property) — a client relying on standard OPC UA semantics (discovering EURange/Quality/etc. as a Property of the Variable) won't find it there. Full fidelity needs: (1) `SchemaNodeValidator.validateParent` to accept `VARIABLE` as a valid parent kind (currently only `FOLDER`/`OBJECT`); (2) the existing `SchemaNode.references()`/`SchemaReference`/`ReferenceType` model (already used by the Manual Schema Editor and NodeSet import, IS-176/177) threaded through scan/discovery so a Variable's real Property/Component edges are captured, not just dropped or flattened; (3) **`SchemaNamespace` (worker-opcua) currently never reads `references()` at all — every materialized reference is hardcoded `Organizes`, so even a user-authored typed reference from the Manual Schema Editor has zero effect on the actual served OPC UA server today.** Wiring this is the bulk of the work: extend `protocol_data_source.proto`'s schema-node message with a reference-type field, thread it through `ScanService.populateSchema`, and rewrite `SchemaNamespace.createNodes()` to materialize the real reference type instead of always `Organizes`.

  **Governance note:** this changes the protocol-neutral model (`backend-specs/01_PROTOCOL_NEUTRAL_MODEL.md`'s "no nested structs / parent must be FOLDER or OBJECT" stance) — per `AGENTS.md`, propose the specific model change and get owner approval before altering that spec or `SchemaNodeValidator`'s parent-kind rule. Depends: IS-188 (already landed; this task is a superset covering the topology-fidelity gap IS-188 explicitly left, plus wiring `references()` materialization that never existed even before IS-188). SPEC: Create From Scan / Manually Create Data Source Schemas
