...
- [x] IS-188 [BE] ✅ [source] Create-from-scan fails on real devices exposing Property-of-Variable nodes (e.g. AnalogItemType `EURange`/`EngineeringUnits`, or a `Quality` property) — `OpcUaDiscovery.browse()` intentionally stops descending into a Variable's `HasProperty` children (to bound scan RPCs) but still emits them as full discovered nodes with the owning Variable as `parentId`; `SchemaNodeValidator.validateParent` rejects any parent whose kind isn't `FOLDER`/`OBJECT`, so `createFromScan`/`applyRescan` throws `parent must be a FOLDER or OBJECT: <nodeId>` and the whole scan is unusable. Property-of-Variable nodes (EURange, EngineeringUnits, EnumStrings, Quality, ...) are metadata about the value, not part of it — skip emitting them as discovered nodes entirely instead of adding them as leaves with an invalid parent kind. Reported against a real device: `parent must be a FOLDER or OBJECT: ns=6;s=MyLevel.Alarm/0:Quality`.
...
- [x] IS-180 [BE+FE] ✅ [gen] Type-aware synthetic authoring and live-value readability — redesign the synthetic profile by data-type family: bounded integer patterns and validation; decimal defaults and presentation for FLOAT32/FLOAT64; boolean-specific controls; fixed text plus random/cycling value lists for STRING and LOCALIZED_TEXT; ISO-8601 DATETIME input; and suitable constant editors for the remaining supported neutral types. Correct the live-value SSE delta consumer so batched updates apply without reloading, and format values/units readably without losing access to exact values. SPEC: Generate Synthetic / Observe Live Data Values
...
- [x] IS-161 [BE] ✅ [recording] Recording carries its own schema snapshot, independent of any data source — Flyway migration adds `recordings.schema_nodes` jsonb (backfilled via a correlated subquery against `schemas`/`schema_nodes`); `RecordingService.create`/`startCapture` capture the current schema's nodes onto the row; `getRecordingSchema`/`RecordingProfiler` now read the recording's own stored snapshot instead of a live `schemas.findByVersion(dataSourceId, ...)` lookup, only 404ing when the snapshot is empty; `RecordingExportManifest`/`RecordingImportExportService` carry `schemaNodes` through export/import (absent on older bundles falls back to empty). SPEC: Store Multiple Recordings And Samples
- [x] IS-162 [BE] ✅ [recording] Recording values list returns empty when schema is unresolvable — `JooqValueTimelineRepository.readPage`/`countFiltered` INNER JOIN `schemas` (keyed on `recordings.data_source_id`/`schema_version`) just to resolve a display-only `path`; since IS-160 made `data_source_id` non-authoritative, any recording whose data source doesn't resolve silently dropped every value row from the Values tab even though `value_timeline` had the data. Fixed: `.join(SCHEMAS)` → `.leftJoin(SCHEMAS)` in both queries; `parameterPath` now gracefully falls back to `null` instead of the row vanishing. SPEC: Store Multiple Recordings And Samples
- [x] IS-163 [BE+FE] ✅ [scan] Real-time scan progress (connecting/connected/scanning + node count) — a create-from-scan against a real OPC UA device with a large/deep address space (e.g. a public test server) sat at a static "Scanning…" for the entire run since `Scan` was a unary gRPC call and `ScanJob`/`ScanJobResponse` only carried a terminal state, no in-flight progress. `Scan` is now server-streaming (`ScanEvent` = `ScanProgress`|`ScanResponse`, matching the existing `Capture`/`ClientEvents` streaming pattern); `OpcUaDiscovery.browse()` reports its node counter via callback; `Supervisor.scan()` stays attached to the worker across the stream (with proper cancellation on interrupt/timeout) instead of one blocking call; `ScanJob` gains a mutable in-flight phase (`CONNECTING`/`CONNECTED`/`SCANNING`) + node count, surfaced through `ScanJobResponse`; frontend wizard polling displays the live phase/count instead of discarding the `RUNNING` poll response. SPEC: Create From Scan
- [x] IS-164 [BE+FE] ✅ [scan] Cancellable scan + configurable timeout — large real address spaces (500+ nodes) could exceed the fixed 120s scan deadline (`WorkerClient.DEFAULT_SCAN_TIMEOUT_SECONDS`), aborting with `DEADLINE_EXCEEDED` with no way to stop early or know total node count in advance (OPC UA browse has no upfront count; total stays unknown until fully crawled). BE: `POST …/data-sources/scan/{jobId}/cancel` interrupts the running scan's thread (`ScanService.cancelScan`, synchronized per-job via a `ScanExecution` holder to avoid the interrupt racing onto a reused pool thread), which unblocks `WorkerClient.scan`'s await, cancels the gRPC stream, and — via `Supervisor.withWorker`'s existing `finally` — tears down the one-shot worker process, actually stopping the browse; job settles as a new `CANCELLED` state; a no-op (not an error) once the job is no longer running. Default scan timeout raised 120s → 1800s (`IOTSIM_SIMULATOR_SCAN_TIMEOUT_SECONDS`) as a safety net now that Stop Scan is the primary way to bound a long scan. FE: "Stop Scan" button in the create-data-source wizard's scan step (`create-data-source-wizard-page.tsx`), enabled only once the job id is known (avoids a silent no-op if clicked in the brief pre-jobId window); progress stays "discovered N nodes" (no total/percentage, since total is unknowable upfront). SPEC: Create From Scan
- [x] IS-165 [BE] ✅ [scan] Stream scan node batches over gRPC + paginate scan nodes REST API — a scan against a server with a huge address space (100k+ nodes) fails with `RESOURCE_EXHAUSTED: gRPC message exceeds maximum size 4194304` because the worker→supervisor `Scan` stream (IS-163) only streams `ScanProgress` ticks; the terminal `ScanEvent` still carries the *entire* discovered node list in one `ScanResponse` message, with no `maxInboundMessageSize` override anywhere. `ScanEvent.oneof` gains a `NodeBatch node_batch` variant (repeated `SchemaNodeMsg`, ~500/batch); `ScanResponse` drops its `nodes` field (metadata-only terminal event: status/truncated/discovered_count/unknown_count/message); `OpcUaProtocolService.scan()` streams node batches as discovered instead of one final blob; `WorkerClient.scan()` accumulates batches client-side into a `ScanOutcome` record. REST: new additive `GET /{jobId}/nodes?cursor=&limit=` reuses the existing `Page<T>` cursor-pagination convention (matches `ProjectController`/`RecordingValuesController`, limit clamped via the shared `PageCursor.clamp`) alongside the existing `GET /{jobId}` (which still returns the full `nodes` list unchanged, for backward compatibility until UI-470 switches the wizard over to the paged endpoint in the same PR that adds its frontend consumer). `backend-specs/05_API_CONTRACT.md` scan section updated to document the new sub-resource. FE half: UI-470. SPEC: Create From Scan
- [x] IS-166 [BE] ✅ [recording] Expose active-capture status so a stuck/orphaned capture is queryable — `RecordingService`'s in-flight-capture tracking was a plain in-memory `Map<String, ActiveCapture>` with no read accessor; the only symptom of an orphaned capture (e.g. left running after a page reload with no Stop) was a rejected `start` with "already running", no way to discover its `recordingId` or stop it deliberately. New `RecordingService.captureStatus(dataSourceId)` returns a `CaptureStatus(capturing, recordingId)` record; new `GET …/data-sources/{id}/recording/status` (`RecordingCaptureController`, `OBSERVE` permission, matching `RecordingController`'s other read endpoints) surfaces it. FE half: UI-476.
- [x] IS-167 [BE] ✅ [recording] Auto-generate a default recording name at capture start — every recording created via live capture (`RecordingService.startCapture`) got `name: null` (there is no UI anywhere to name a recording), so screens fell back inconsistently (source name, truncated id, ...) and multiple recordings from the same source were indistinguishable in lists. `startCapture` now generates a default name of `"<source name> — <date>, <time>"` (mirrors the date format already used in the frontend) instead of passing `null`.
- [x] IS-168 [BE] ✅ [gen] Allow CONSTANT pattern for structural/identifier data types in synthetic generation — creating a Synthetic source that reused a real device's discovered schema failed with "synthetic generation does not support NODE_ID in v1" for any structural/identifier-typed node (`GUID`, `STATUS_CODE`, `QUALIFIED_NAME`, `NODE_ID`, `EXPANDED_NODE_ID`, `XML_ELEMENT`, `BYTES`, `DATETIME` — unconditionally rejected by `SyntheticVariable`). A dynamic pattern has no physical meaning for these (structural OPC UA plumbing, not measured signals — a real device wouldn't vary a NodeId reference over time either), but `CONSTANT` is exactly what they already are on a real device. `SyntheticPattern.Constant.value` widened `double` → `Object` (mirrors `EnumCycle`/`StepSequence`'s existing `Object` payloads); `SyntheticVariable` only rejects these 8 types for non-`Constant` patterns now; `SyntheticValueCoercion` handles all 8 for a `Constant`-sourced value (GUID validated as a UUID string, identifier/text types pass through as `String`, STATUS_CODE/DATETIME via the existing numeric-to-`Long` path, BYTES accepts `byte[]`); `PatternSpec`/`SyntheticConfigMapper` gained `stringValue`/`bytesValueBase64` fields (additive, a legacy 8-arg `PatternSpec` constructor keeps every existing numeric-only call site compiling unchanged) to carry a non-numeric constant over JSON. Type-appropriate synthetic defaults (not seeded from a real device's captured value — that plumbing doesn't exist in the schema-reuse path). FE half: UI-482.
- [x] IS-169 [BE] ✅ [recording] Default a name for scan-created recordings, mirroring IS-167's live-capture fix — `RecordingService.create()` (used by the "Create Recording" scan-and-record wizard) always got `name: null` since no caller passes one, so multiple recordings from the same source on the same day were indistinguishable everywhere they're listed (Import Data wizard's recording picker, Recordings page) — they all fell back to the source's name. `create()` now generates the same `"<source name> — <date>, <time>"` default as `startCapture` when no name is given; the shared helper is renamed `defaultRecordingName`. FE half (show time, not just date, in the Import picker): UI-485.
- [x] IS-170 [BE] ✅ [scan] Rescan an existing SCAN-basis data source's real endpoint (one cohesive FE+BE feature) — once a SCAN-basis source was created, there was no way to re-discover its real device's structure again; only the create wizard could scan. BE: `ScanService.startRescan(projectId, dataSourceId)` loads the existing source (requires `basis==SCAN` + a non-blank `realDeviceEndpoint`), reuses its stored connection credentials via a newly-injected `CredentialStore` (same reuse pattern as `RecordingService.startCapture`), and delegates to the existing `startScan`; `ScanService.applyRescan(...)` reuses the existing `populateSchema` mapping + `SchemaService.save` to persist the discovered structure as a new schema version on the *existing* source id (unlike `createFromScan`, no new source is created). New `DataSourceController` endpoints: `POST /{id}/rescan` (202 + job id, reusing the existing `GET /data-sources/scan/{jobId}` poll endpoint) and `POST /{id}/rescan/{jobId}/apply` (200 + updated `DataSourceResponse`). FE: a "Rescan tags" panel on the Settings tab (SCAN-basis only) reuses the create wizard's `UnknownNodesList`/`fetchAllScanNodes` for unknown-typed-node resolution; new `applyRescan` store action mirrors `duplicateDataSource`/`stopDataSource`.
- [x] IS-174 [SDLC] ✅ Manual Schema design: governance doc updates + task registration — adds the approved ManualSchema design (a reusable, project-scoped, protocol-scoped structure artifact, symmetric to Recording, consumed only via a synthetic source's manualSchemaId) to `backend-specs/01_PROTOCOL_NEUTRAL_MODEL.md` (§6 where-used), `03_DOMAIN_MODEL.md` (new entity), `04_DB_SCHEMA.md` (`manual_schemas` table), `05_API_CONTRACT.md` (`/manual-schemas` endpoints + synthetic-source field), and `MEMORY.md` glossary; registers `IS-171`–`IS-173` and `UI-489`–`UI-491` in the task catalogs (Wave D). SPEC: Manually Create Data Source Schemas
- [x] IS-171 [BE] ✅ [schema] ManualSchema entity + persistence + Flyway migration — new `manual_schemas` table (`project_id`, `protocol`, `name`, `description`, `nodes` jsonb, audit columns, `version`), mirroring the `recordings.schema_nodes` jsonb precedent (IS-161) rather than normalized `schema_nodes` rows. `ManualSchemaRow`/`ManualSchemaRepository` in `persistence`, incl. `findByProjectPaged`. — 03/04, SPEC: Manually Create Data Source Schemas
- [x] IS-172 [BE] ✅ [schema] ManualSchemaService + REST API (CRUD + duplicate) — `ManualSchemaService`: list(paged)/get/create/save(in-place)/duplicate(save-as)/delete, reusing `SchemaService`'s node validation (unique nodeId/path). `ManualSchemaController` under `/api/v1/projects/{pid}/manual-schemas` per the new `05_API_CONTRACT.md` section. Depends: IS-171. — 05, SPEC: Manually Create Data Source Schemas
- [x] IS-173 [BE] ✅ [source] Wire `manualSchemaId` into synthetic source creation — extend `CreateSyntheticSourceRequest`/`SyntheticSourceService.create` with `manualSchemaId` (mutually exclusive with the existing `schemaFromSourceId`), copying the `ManualSchema`'s nodes by snapshot the same way `copiedSchemaNodes` does today for `schemaFromSourceId` (IS-145). Depends: IS-171. FE half: UI-491. — SPEC: Manually Create Data Source Schemas / Generate Synthetic Data
- [x] IS-175 [BE] ✅ [runtime] Materialize Manual Schema hierarchy in the OPC UA worker — `SchemaNamespace` currently creates each variable directly under the Objects folder and ignores `FOLDER` nodes/`parentId`, so a nested Manual Schema is flattened at runtime. Create browseable OPC UA folders/objects and attach each typed variable under its declared parent; reject invalid parent relationships instead of silently flattening them. Depends: IS-173. FE half: UI-492. SPEC: Manually Create Data Source Schemas
- [x] IS-176 [BE] ✅ [schema] OPC UA address-space model — extend schemas with Objects, Methods, typed references, array dimensions and type definitions while preserving existing folders and variables. Validate inconsistent parents, missing reference targets and cyclic parent relationships. Depends: IS-175. — 01/04/05
- [x] IS-177 [BE] ✅ [schema] Import OPC UA NodeSet XML into Manual Schemas — parse supported NodeSet objects, variables, methods and hierarchical references into a reusable ManualSchema. Report unsupported definitions explicitly without flattening or silently dropping nodes; retain imported hierarchy for the OPC UA worker. Depends: IS-176. — 05, SPEC: Manually Create Data Source Schemas
- [x] IS-178 [BE] ✅ [observ] Evidence manifest never gets `endedAt` for non-live runs — `SyntheticRunService`, `ReplayService`, `ScenarioRunService`, and `ScenarioLiveRunService` write the evidence manifest once at run start and never stamp it with `endedAt`/full run metadata on completion, unlike `ReplayLiveRunService`/`SyntheticLiveRunService` (fixed in IS-157). Leaves the Evidence detail page permanently "Not ready" with empty KIND/INITIATOR/SOURCES and a disabled Export button for scenario/synthetic/replay (non-live) runs, even though the underlying run is `COMPLETED`.
- [x] IS-179 [BE] ✅ [observ] Evidence manifest `endedAt` never stamped on manual stop (live runs) — `SyntheticLiveRunService.stopIfLive` and `ReplayLiveRunService.softStampEvidence` write `endedAt: null` on the manual-stop path (`RunService.stop`); `ReplayLiveRunService` never stamps a real `endedAt` at all (auto-completion included). Leaves the Evidence detail page "Not ready" for manually-stopped live synthetic/replay runs even though `RunRow.endedAt` is set correctly — the live-service counterpart the IS-157/IS-178 pattern missed.
- [x] IS-181 [BE] ⬜ [gen] Random DATETIME and GUID synthetic values — extend type-aware random generation with DATETIME values within an ISO-8601 UTC range and a per-update GUID, preserving seeded determinism and rejecting invalid values early. FE half: UI-498. SPEC: Generate Synthetic
- [x] IS-182 [BE] ✅ [observ] Run completion/stop/failure never recorded in runtime_events — `SyntheticRunService`, `SyntheticLiveRunService`, `ReplayService`, `ReplayLiveRunService`, `ScenarioRunService`, and `ScenarioLiveRunService`'s run-level terminal transitions (COMPLETED/STOPPED/FAILED) are tracked in the `runs` table but never appended to `runtime_events`, so the per-source Events tab (reads only `runtime_events`) never shows anything about a run finishing. Adds a `RUN_COMPLETED`/`RUN_STOPPED`/`RUN_FAILED` event (advisory, swallow-and-continue) alongside every terminal `runs.end(...)` write, including the shared manual-stop path in `RunService.stop`. FE half: UI-501.
- [x] IS-183 [BE] ✅ [schema] Custom DataType node class — new NodeKind + typeDefinition wiring — adds `NodeKind.DATA_TYPE` (a user-authored custom structured type, e.g. a "Vector3D" struct with x/y/z FLOAT64 members) with an ordered `List<DataTypeMember>` on `SchemaNode`, and a `dataTypeNodeId` field so a VARIABLE can reference a custom DATA_TYPE node instead of only a primitive `DataType` enum value; `SchemaNodeValidator` requires DATA_TYPE nodes to be top-level (no parentId) and validates member uniqueness/targets/one-level nesting. DTOs round-trip through `ManualSchemaController`/`SchemaController`/`DataSourceController`. Worker-runtime materialization (Milo `UaDataTypeNode`, proto contract) deferred to IS-184. Depends: IS-176.
- [x] IS-192 [BE+FE] ✅ [schema] Native OPC UA type-fidelity foundation — preserve native DataType NodeIds, enum literals, structure fields, default binary encodings, and typed values through scan, schema persistence, capture/replay, and the initial manual-editor catalog. Materialize native enums and preserve binary structure ExtensionObjects; status-only/opaque values fail explicitly rather than being coerced. The complete standard/vendor catalog and general executable materialization move to IS-194. Builds on IS-183. SPEC: Preserve OPC UA Type Fidelity
- [x] IS-194 [BE+FE] ✅ [schema] Complete OPC UA native type catalog and executable materialization — deliver the complete versioned catalog of standard structures, unions, enums, and option sets; transitively resolve and retain vendor definitions during scan; persist catalog snapshots with schemas, recordings, and exports; materialize supported definitions/codecs in the OPC UA worker; recursively capture/replay typed values; generate the manual editor catalog from the same source; and verify scan → save → serve/re-scan plus capture → replay against deterministic fixtures and a public endpoint. Opaque definitions retain their identity and produce actionable diagnostics. Depends: IS-192. SPEC: Preserve OPC UA Type Fidelity
- [x] IS-195 [BE] ✅ [scan] Preserve duplicate OPC UA BrowsePaths during scan — OPC UA NodeId is the source identity, but a server may expose separate nodes through the same display/BrowsePath (for example `Data/Conditions/CycleComplete`). Import every discovered node and retain its original NodeId; keep a path unchanged when it is unique and derive a deterministic, NodeId-marked schema path only for collisions, including their descendants. Depends: IS-194. SPEC: Create Data Source From Real Source Scan
- [x] IS-196 [BE] ✅ [recording] Capture OPC UA native structures with executable encodings — supervisor capture preflight must accept a scanned native structure such as `Range` when its declaration provides a default binary encoding. Retain the raw ExtensionObject bytes and encoding identity; structures whose executable encoding is absent or invalid must still fail explicitly. Depends: IS-194. SPEC: Capture Real Device Data
- [x] IS-202 [BE+FE] ✅ [schema] Preserve OPC UA methods in scan, manual schemas, and runtime — scan Method nodes with their parent/reference relationship; allow explicit manual Method authoring; materialize methods in the OPC UA runtime and return `Bad_NotImplemented` rather than silently omitting unsupported behavior. Depends: IS-194. SPEC: Create Data Source From Real Source Scan
- [x] IS-201 [BE] ✅ [recording] Preserve OPC UA array values through capture and replay — carry variable rank and dimensions over the worker contract; encode primitive, identifier/text, enum and native structure arrays as canonical TREE values; validate rank/shape; and materialize typed arrays or matrices on replay without dropping element ordering or binary ExtensionObject bodies. Depends: IS-194, IS-196. SPEC: Capture Real Device Data
- [x] IS-193 [BE] ✅ [schema] Persist and inspect native OPC UA structured types — preserve native DataType NodeIds plus structured and enum declarations across schema versions; import OPC UA 1.04 structure definitions during scan; expose declarations through schema/manual-editor APIs and UI. Excludes complete built-in catalog coverage, enum/vendor discovery, and worker materialization/replay, which remain in IS-192. Depends: IS-183. SPEC: Preserve OPC UA Type Fidelity
- [x] IS-197 [BE] ✅ [schema] Abstract types (BaseDataType/Variant/UInteger) — capture and replay with native type discriminator preservation; test fixtures; integration tests. Depends: IS-194. SPEC: Preserve OPC UA Type Fidelity
- [x] IS-198 [BE] 🟡 [schema] OptionSet (named bit flags) — correct capture/storage/replay of bit-set semantics; named-bit resolution during worker materialization; bit-flag struct encoding. Depends: IS-194. SPEC: Preserve OPC UA Type Fidelity
- [ ] IS-199 [BE] 🟡 [schema] Opaque/vendor types pre-run validation — detect unsupported type definitions at schema save or run start time (not mid-execution); structured diagnostics with recovery hints for each unsupported type; validation gates in SchemaService, RecordingService, ReplayService, SyntheticRunService. Depends: IS-194. SPEC: Preserve OPC UA Type Fidelity
- [x] IS-200 [BE] ✅ [schema] Manual OPC UA native schemas — synthetic source generation from native type structures; discovery and codification of vendor-specific catalogs via NodeSet import; complete fixture suite. Depends: IS-177. SPEC: Preserve OPC UA Type Fidelity / Manually Create Data Source Schemas
- [x] IS-185 [BE] ✅ [observ] Evidence value-timeline empty for Synthetic runs — `EvidenceService.assemble()` only populated `valueTimeline` when the evidence manifest had a `recordingId`; `SyntheticLiveRunService`/`SyntheticRunService` never persisted generated values anywhere, so exported evidence for Synthetic runs always shipped an empty `value-timeline.json` even though `backend-specs/06_ARTIFACT_FORMATS.md` requires value timelines regardless of run kind. New `run_value_timeline` table (keyed by `run_id`, mirrors `value_timeline`'s shape) + `RunValueTimelineRepository`; `SyntheticLiveRunService.tickOne`/`SyntheticRunService.run` append every generated batch; `EvidenceService.assemble()` falls back to it when `recordingId` is null. Scenario-run evidence still ships empty (its steps each get their own per-step run+evidence; aggregating a scenario's own evidence timeline from its child runs is a separate piece) — tracked as IS-186. SPEC: Export Run Evidence
- [ ] IS-186 [BE] ⬜ [observ] Evidence value-timeline empty for Scenario runs — same gap as IS-185 but for `ScenarioRunService`/`ScenarioLiveRunService`: a scenario's own evidence has no `recordingId`, and its `runId` is the parent scenario run, not any child step's run, so `RunValueTimelineRepository.readAll(runId)` (added in IS-185) finds nothing even after a Synthetic/Replay step ran. Needs `RunRepository` to expose child runs by `parentRunId` (no such lookup exists yet) so `EvidenceService.assemble()` can concatenate each child run's value timeline (from `value_timeline` via its `recordingId` for Replay steps, or `run_value_timeline` for Synthetic steps) into the scenario evidence's own `value-timeline.json`. Depends: IS-185. SPEC: Export Run Evidence
- [x] IS-187 [BE] ✅ [observ] Evidence status never leaves CAPTURING when its run finishes — `evidence.status` (`CAPTURING | READY | PARTIAL | EXPORT_FAILED`) is only ever written by `EvidenceService.export()` — every `runs.end(...)` terminal write across `SyntheticRunService`, `SyntheticLiveRunService`, `ReplayService`, `ReplayLiveRunService`, `ScenarioRunService`, `ScenarioLiveRunService`, and the shared manual-stop path in `RunService.stop` stamps the manifest's `endedAt` but never touches `evidence.status`. Per `frontend/docs/UI_SCREEN_SPECS.md` ("Key states: capturing; ready; partial; export failed"), status should reflect the run's own completion, independent of whether a bundle was ever exported — instead every completed/stopped/failed run's evidence sits permanently as `CAPTURING` ("In progress") until the user manually clicks Export. Add a shared helper (mirrors `RunCompletionEvents`) that flips `CAPTURING → READY` (COMPLETED) or `CAPTURING → PARTIAL` (STOPPED/FAILED) at every terminal write, without clobbering a status an explicit export already set. SPEC: Export Run Evidence
