...
- [ ] IS-162 [BE] ⬜ [recording] Recording values list returns empty when schema is unresolvable — `JooqValueTimelineRepository.readPage`/`countFiltered` INNER JOIN `schemas` (keyed on `recordings.data_source_id`/`schema_version`) just to resolve a display-only `path`; since IS-160 made `data_source_id` non-authoritative, any recording whose data source doesn't resolve silently drops every value row from the Values tab even though `value_timeline` has the data. Fix: `.join(SCHEMAS)` → `.leftJoin(SCHEMAS)` in both queries. SPEC: Store Multiple Recordings And Samples
