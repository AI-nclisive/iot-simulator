...
Snapshot: **build green.** 111 done · 17 todo (128 total). Live status is the board; this line is a periodic snapshot.
...
- [ ] IS-128 [BE] Worker external-host bind: make the protocol worker's bind interface + advertised hostname configurable at deployment level so a simulated source is reachable off-host. New `iotsim.simulator.bind-address` (default `0.0.0.0`); existing `iotsim.simulator.advertised-host` now also drives the worker's advertised OPC UA endpoint (so it matches `serveUrl`). Transported via the existing `ConfigureRequest.options` map (no proto change); loopback fallback keeps existing worker tests green. Follows memory `worker-bind-external-access`; builds on IS-127. Spec: `docs/superpowers/specs/2026-07-02-is-128-worker-external-bind-design.md`.
