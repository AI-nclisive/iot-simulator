...
- [ ] IS-192 [BE+FE] ⬜ [schema] Preserve and materialize OPC UA structured DataTypes — replace the v1 no-nested-structs restriction with a schema-owned native type catalog. Scan imports standard, enum, structure, and vendor definitions; schemas, recordings, and exports retain their identity and typed value shape; the OPC UA worker materializes executable definitions and rejects only an explicitly opaque/unmaterializable definition; the editor lets users inspect and author the same types. Builds on IS-183. SPEC: Preserve OPC UA Type Fidelity
