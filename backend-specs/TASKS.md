...
- [ ] IS-170 [BE] ⬜ [schema] ManualSchema entity + persistence + Flyway migration — new `manual_schemas` table (`project_id`, `protocol`, `name`, `description`, `nodes` jsonb, audit columns, `version`), mirroring the `recordings.schema_nodes` jsonb precedent (IS-161) rather than normalized `schema_nodes` rows. `ManualSchemaRow`/`ManualSchemaRepository` in `persistence`. — 03/04, SPEC: Manually Create Data Source Schemas
- [ ] IS-171 [BE] ⬜ [schema] ManualSchemaService + REST API (CRUD + duplicate) — `ManualSchemaService`: list/get/create/save(in-place)/duplicate(save-as)/delete, reusing `SchemaService`'s node validation (unique nodeId/path). `ManualSchemaController` under `/api/v1/projects/{pid}/manual-schemas` per the new `05_API_CONTRACT.md` section. Depends: IS-170. — 05, SPEC: Manually Create Data Source Schemas
- [ ] IS-172 [BE] ⬜ [source] Wire `manualSchemaId` into synthetic source creation — extend `CreateSyntheticSourceRequest`/`SyntheticSourceService.create` with `manualSchemaId` (mutually exclusive with the existing `schemaFromSourceId`), copying the `ManualSchema`'s nodes by snapshot the same way `copiedSchemaNodes` does today for `schemaFromSourceId` (IS-145). Depends: IS-170. FE half: UI-488. — SPEC: Manually Create Data Source Schemas / Generate Synthetic Data
