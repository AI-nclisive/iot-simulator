...
export function DataSourceSchemaEditor({ source }: { source: DataSourceRow }) {
  ...
  const lockState: EditLockState = mockSourceLock
  ...
  if (
    ...
    editBuffer.min &&
    editBuffer.max &&
    Number(editBuffer.min) > Number(editBuffer.max)
  ) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          disabled={!hasUnsavedChanges || isLockedByOther || validationIssues.length > 0}
          type="button"
          onClick={handleSave}
        >
          Save schema
