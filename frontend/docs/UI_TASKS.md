...
- [x] `UI-134` Fix scenario status reset — preserve runState when navigating away from run view
  Goal: scenario list showed "Not running" immediately after navigating away from the run view, even when the scenario was still running. Root cause: clearLiveRun unconditionally reset runState on SSE cleanup.
  Fix: onRunFinished now maps terminal SSE state to ScenarioRunState and writes it to scenarios[]; clearLiveRun only removes the liveRuns entry without touching runState.
  Done when: navigating away from run view while a scenario is running keeps its runState; run-finished SSE event updates runState correctly; typecheck passes.
