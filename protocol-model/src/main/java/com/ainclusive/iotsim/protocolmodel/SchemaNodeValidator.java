...
public final class SchemaNodeValidator {
    ...
    public static void validate(List<SchemaNode> nodes) {
        ...
        for (SchemaNode node : nodes) {
            ...
            if (!paths.add(node.path())) throw new IllegalArgumentException("duplicate node path: " + node.path());
        }
        for (SchemaNode node : nodes) {
        ...
        for (SchemaNode node : nodes) assertAcyclic(node, byId, new HashSet<>());
    }
    ...
    private static void validateParent(SchemaNode node, Map<String, SchemaNode> byId) {
        ...
        if (parent == null) throw new IllegalArgumentException("parent does not exist: " + node.parentId());
    ...
    private static void assertAcyclic(SchemaNode node, Map<String, SchemaNode> byId, Set<String> visiting) {
        if (node.parentId() == null) return;
        if (!visiting.add(node.nodeId())) throw new IllegalArgumentException("cyclic parent relationship at: " + node.nodeId());
        SchemaNode parent = byId.get(node.parentId());
        if (parent != null) assertAcyclic(parent, byId, visiting);
