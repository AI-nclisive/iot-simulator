...
mode via the `Capture` RPC: the worker connects to the real endpoint,
subscribes to the schema's variable nodes (every change, no sampling), and
streams neutral value batches back until the call is cancelled. This is
symmetric with `Scan`/`TestConnection`, keeping all protocol-specific code in
the worker.
...
## Known gaps
- `SchemaNodeMsg` in `worker-contract/src/main/proto/iotsim/worker/v1/protocol_data_source.proto`
  does not yet carry `arrayDimensions`, `typeDefinition`, or `DATA_TYPE`
  member fields, so schemas using the newer node kinds from the
  `protocol-model` capability cannot be sent to a worker via `Configure` yet.
