...
Manual Schemas, Recordings & Samples, Scenarios, Evidence, Activity,
Settings, Admin. Labels SHALL use product language only, never backend or
architecture terms.

#### Scenario: Every primary route has a nav entry
- **WHEN** a project-scoped route is reachable in the app (including
  `Manual Schemas` and `Activity`)
- **THEN** it has a corresponding entry in the primary navigation - a route
  reachable only by typing its URL is a gap, not an acceptable alternative
...
## Known gaps
- The current navigation implementation omits an `Activity` link even though
  the `/activity` route exists and is fully implemented - it is reachable
  only by direct URL today. This is a real gap against this spec's
  every-route-has-a-nav-entry requirement, not a documentation error;
  tracked as a pending fix, not part of this baseline's claimed behavior.
