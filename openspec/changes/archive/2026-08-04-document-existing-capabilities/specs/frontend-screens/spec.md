...
`Recordings & Samples`, `Scenarios`, `Scenario Run View`, `Evidence List`,
`Evidence Detail`, `Activity View`, `Settings`, `Admin UI`; plus
account-entry pages `Login` (shared mode) and `Project Entry`.
...
## Known gaps
- No dedicated screen spec exists yet for the retention/cleanup admin flow's
  concrete UI (the `Retention & Cleanup` shared-surface *contract* is
  captured above; its full page-level design is still pending, matching
  `UI_TASKS.md`'s own tracking of that item as not yet built).
