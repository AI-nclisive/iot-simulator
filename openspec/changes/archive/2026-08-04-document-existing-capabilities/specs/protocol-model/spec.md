...
## Known gaps
- The `worker-contract` gRPC `SchemaNodeMsg` does not yet carry
  `arrayDimensions`, `typeDefinition`, or a `DATA_TYPE`-shaped `members` list
  (see the `worker-contract` capability) — the newer node kinds cannot cross
  the wire to a worker yet; this is scoped for a follow-up change, not this
  baseline.
- The `schema_nodes.kind` database check constraint currently allows
  `FOLDER | OBJECT | VARIABLE | METHOD` only; `DATA_TYPE` is not yet in that
  constraint (see the `db-schema` capability), so persisting a `DATA_TYPE`
  node through the normalized `schemas`/`schema_nodes` tables (as opposed to
  a jsonb-backed `manual_schemas` row) needs that constraint widened first.
