...
Two directories carry the whole picture:
...
- **Never hand-edit `openspec/specs/*.md`.** It is generated by archiving a
  change. Edit the delta under `openspec/changes/<id>-<slug>/specs/`, then
  archive.
- **Only `### Requirement:` blocks survive archive.** Other prose in a delta is
  dropped silently, so write a limitation as normative requirement text rather
  than a footnote.

CI enforces the pairing: a PR whose body says `Implements: IS-XXX` must add the
matching `openspec/changes/archive/<date>-is-XXX-<slug>/` in the same PR
(`.github/workflows/ci.yml` → `catalog-sync`). Full rules live in
